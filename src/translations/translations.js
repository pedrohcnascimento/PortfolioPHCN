export const translations = {
  en: {
    hero: {
      title: "Pedro Henrique Carvalho do Nascimento",
      subtitle: "Back-end Developer & Systems Development Student",
      location: "São Paulo, Brazil",
      scroll: "Scroll Down",
    },
    about: {
      title: "About Me",
      text1:
        "I am a dedicated and enthusiastic young professional, currently pursuing a technical degree in Systems Development. I am fluent in English and have solid knowledge of back-end programming. In addition, I have practical experience with version control using Git/GitHub and familiarity with Microsoft Copilot, as well as programming tools such as IntelliJ and VSCode.",
      text2:
        "I have a preference for back-end development, but as you can see, I am also able to develop simple front-end projects. My main programming languages are Java, MySQL, and JavaScript. The development architectures I usually use are DDD (Domain Driven Design) and TDD (Test Driven Design), and I frequently use the Spring Boot framework for Java development. Currently, I am looking for my first professional opportunity in the technology field, where I can apply my ability to learn quickly and my passion for technology.",
      interestsTitle: "Interests",
      interestsText:
        "I am very passionate about music; at the moment I am studying music theory and singing in my church choir. I also enjoy playing games and learning new things in general, especially when they are related to technology. Additionally, I play tabletop RPGs — I really enjoy creating stories, and it's fun to challenge my friends while also being challenged by them.",
    },
    skills: {
      title: "Technical Skills",
      languages: "Programming Languages",
      frameworks: "Frameworks & Libraries",
      tools: "Tools & Technologies",
      methodologies: "Methodologies",
    },
    experience: {
      title: "Experience",
      professional: "Professional",
      academic: "Academic",
      viewProject: "View Project →",
      backend: "Backend →",
      frontend: "Frontend →",
      items: [
        {
          type: "professional",
          title: "Project Aurora",
          company: "Freelance (Data Curve.ai)",
          period: "August 2025 - October 2025",
          description:
            "Freelance project using Python to create real-world software development problems and solutions. An AI system would attempt to solve these problems 10 times, with a target success rate of at most 30%. Each properly executed problem was saved in the company's database and later sold to other companies interested in training or improving their AIs."
        },
        {
          type: "academic",
          title: "Inpark",
          company: "SENAI Technical Course Project",
          period: "July 2025 - Present",
          description:
            "Final technical course project with 6 members. Our solution operates on two fronts: simplifying the daily routine of drivers looking for parking quickly and with reliable information, and driving the growth and digitalization of local parking facilities. I was responsible for the project idea, team organization, and development of tests (both back-end and front-end)."
        },
      ],
    },
    education: {
      title: "Education & Courses",
      inProgress: "In-Progress",
      completed: "Completed",
      items: [
        {
          title: "Analysis and Software Development",
          institution: "FIAP",
          period: "Expected completion: December 2027",
          status: "In-Progress",
        },
        {
          title: "Technical Course in Software Development",
          institution: "SENAI Anchieta School",
          period: "Completed: December 2025",
          status: "Completed",
        },
        {
          title: "Fluent English",
          institution: "EF Education First",
          period: "Completed: November 2024",
          status: "Completed",
        },
      ],
    },
    projects: {
      title: "Projects",
      viewGithub: "View on GitHub →",
      items: [
        {
          title: "Project Aurora",
          description: "Exercises for AI training databases",
          link: "https://github.com/pedrohcnascimento/ProjectAurora"
        },
        {
          title: "Inpark Backend",
          description: "Backend for a parking management system",
          link: "https://github.com/Arthur2060/Inpark-backend"
        },
        {
          title: "Inpark Frontend",
          description: "Frontend for a parking management system",
          link: "https://github.com/DiegoGenuino/frontend-inpark"
        },
        {
          title: "Personal Portfolio",
          description: "Want to see how this portfolio was made? Check out the code on GitHub!",
          link: "https://github.com/pedrohcnascimento/PortfolioPHCN"
        }
      ],
    },
    contact: { title: "Contact Information", location: "Location", resume: "Resume", download: "Download PDF" },
    menu: {
      home: "Home",
      about: "About",
      skills: "Skills",
      experience: "Experience",
      education: "Education",
      projects: "Projects",
      contact: "Contact"
    },
  },
  pt: {
    hero: {
      title: "Pedro Henrique Carvalho do Nascimento",
      subtitle:
        "Desenvolvedor Full-Stack & Estudante de Desenvolvimento de Sistemas",
      location: "São Paulo, Brasil",
      scroll: "Role para baixo",
    },
    about: {
      title: "Sobre Mim",
      text1:
        "Sou um jovem profissional dedicado e entusiasmado, atualmente cursando uma formação técnica em Desenvolvimento de Sistemas. Sou fluente em inglês e possuo sólido conhecimento em programação back-end. Além disso, tenho experiência prática com controle de versão utilizando Git/GitHub e familiaridade com Microsoft Copilot, bem como ferramentas de programação como IntelliJ e VSCode.",
      text2:
        "Tenho preferência pelo desenvolvimento back-end, mas como você pode ver, também consigo desenvolver projetos simples de front-end, minhas principais linguagens são: Java, MySQL e JavaScript; a arquitetura de desenvolvimento que costumo utilizar é a DDD (Domain Driven Design) ou TDD(Test Driven Design) e costumo usar bastante o framework SpringBoot para desenvolvimento Java. Atualmente, estou em busca da minha primeira oportunidade profissional na área de tecnologia, onde eu possa aplicar minha facilidade de aprendizado e minha paixão por tecnologia.",
      interestsTitle: "Interesses",
      interestsText:
        "Sou apaixonado por música, no momento estou estudando teoria musical e cantando no coral da minha igreja. Também gosto de jogar e de aprender coisas novas no geral, especialmente quando estão relacionadas à tecnologia. Além disso, jogo RPG de mesa — gosto muito de criar histórias, e é divertido desafiar meus amigos enquanto também sou desafiado por eles.",
    },
    skills: {
      title: "Habilidades Técnicas",
      languages: "Linguagens de Programação",
      frameworks: "Frameworks e Bibliotecas",
      tools: "Ferramentas e Tecnologias",
      methodologies: "Metodologias",
      concepts: "Conceitos Fundamentais",
    },
    experience: {
      title: "Experiência",
      professional: "Profissional",
      academic: "Acadêmica",
      viewProject: "Ver Projeto →",
      backend: "Backend →",
      frontend: "Frontend →",
      items: [
        {
          type: "profissional",
          title: "Project Aurora",
          company: "Freelance (Data Curve.ai)",
          period: "Agosto 2025 - Outubro 2025",
          description:
            "Projeto freelance utilizando Python para criar problemas e soluções reais de desenvolvimento de software. Um sistema de IA tentava resolver esses problemas 10 vezes, com uma taxa de sucesso alvo de no máximo 30%. Cada problema corretamente executado era salvo no banco de dados da empresa e posteriormente vendido para outras empresas interessadas em treinar ou aprimorar suas IAs.",
        },
        {
          type: "academica",
          title: "Inpark",
          company: "Projeto do Curso Técnico SENAI",
          period: "Julho 2025 - Presente",
          description:
            "Projeto final do curso técnico com 6 integrantes. Nossa solução atua em duas frentes: simplificar a rotina de motoristas que buscam estacionamento de forma rápida e confiável e impulsionar o crescimento e a digitalização de estacionamentos locais. Fui responsável pela ideia do projeto, organização da equipe e desenvolvimento de testes (tanto back-end quanto front-end).",
        },
      ],
    },
    education: {
      title: "Educação e Cursos",
      inProgress: "Em andamento",
      completed: "Concluído",
      items: [
        {
          title: "Análise e Desenvolvimento de Sistemas",
          institution: "FIAP",
          period: "Conclusão prevista: Dezembro 2027",
          status: "Em-andamento",
        },
        {
          title: "Curso Técnico em Desenvolvimento de Sistemas",
          institution: "Escola SENAI Anchieta",
          period: "Concluído: Dezembro 2025",
          status: "Concluido",
        },
        {
          title: "Inglês Fluente",
          institution: "EF Education First",
          period: "Concluído: Novembro 2024",
          status: "Concluido",
        },
      ],
    },
    projects: {
      title: "Projetos",
      viewGithub: "Ver no GitHub →",
      items: [
        {
          title: "Project Aurora",
          description: "Exercícios para banco de dados de treinamento de IA",
          link: "https://github.com/pedrohcnascimento/ProjectAurora"
        },
        {
          title: "Inpark Backend",
          description:
            "Backend para um sistema de gerenciamento de estacionamentos",
          link: "https://github.com/Arthur2060/Inpark-backend"
        },
        {
          title: "Inpark Frontend",
          description:
            "Frontend para um sistema de gerenciamento de estacionamentos",
          link: "https://github.com/DiegoGenuino/frontend-inpark"
        },
        {
          title: "Portfólio Pessoal",
          description: "Quer ver como este portfólio foi feito? Confira o código no GitHub!",
          link: "https://github.com/pedrohcnascimento/PortfolioPHCN"
        }
      ],
    },
    contact: { title: "Informações de Contato", location: "Localização", resume: "Currículo", download: "Baixar PDF" },
    menu: {
      home: "Início",
      about: "Sobre",
      skills: "Habilidades",
      experience: "Experiência",
      education: "Educação",
      projects: "Projetos",
      contact: "Contato"
    },
  },
};
